<template>
  <div class="wrapper" @mousemove="playParallax">
    <MovingIcons />
    <MyForm @submit.prevent v-if="authUser.formShow">
      <div class="logo"></div>
      <div style="margin:0">
        <label for="login" class="label">Логин</label>
        <MyInput v-model="authUser.login" type="text" name="login" class="input" @input="inputUpdate" />
        <div class="blockPassword">
          <label for="password" class="label">Пароль</label>
          <MyInput v-model="authUser.password" :type="authUser.showPassword ? 'text' : 'password'" name="password" class="input" id="password" />
      ...
      <MyButton @click="submitResult" id="btn">Войти</MyButton>
    </MyForm>
    <MyForm v-if="authUser.passwordRecovery" style="min-height:370px">
      <div>
        <div class="logo"></div>
        ...
        <MyInput v-model="authUser.email" type="text" name="email" class="input" @input="inputUpdate" />
        <MyButton @click="authUser.getPassword" id="btn">Запросить</MyButton>
      </div>
    </MyForm>
  </div>
</template>
